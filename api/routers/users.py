...
async def get_current_active_user(
    ...
    print("get_current_active_user is invoked!!!")
    print(current_user)
...
# @router.get("/token_validate?token={token}", response_model=bool)
@router.get("/token_validate")
...
async def read_own_items(
    ...
    """
    Example:
        curl -X POST -H "content-type: application/x-www-form-urlencoded" "http://localhost:9000/users/token" -d "username=alice" -d "password=password123"
        curl -X GET -H "Authorization: Bearer <token>" "http://localhost:9000/users/me/items"
    """
